 ...
 * @cdk.githash
 */
public class NNRing extends NNAtomContainer implements IRing {
    ...
    public NNRing(int ringSize, String elementSymbol) {
        ...
        atoms[0] = getBuilder().newInstance(IAtom.class, elementSymbol);
        for (int i = 1; i < ringSize; i++) {
            atoms[i] = getBuilder().newInstance(IAtom.class, elementSymbol);
            super.bonds[i-1] = getBuilder().newInstance(IBond.class, atoms[i - 1], atoms[i], IBond.Order.SINGLE);
        }
        super.bonds[ringSize-1] = getBuilder().newInstance(IBond.class, atoms[ringSize - 1], atoms[0], IBond.Order.SINGLE);
    }

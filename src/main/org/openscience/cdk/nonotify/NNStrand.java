 ...
 * @cdk.githash
 */
public class NNStrand extends NNAtomContainer implements IStrand {

	private static final long serialVersionUID = -1600146220587659667L;
    ...
    public NNStrand() {
        ...
        IMonomer oMonomer = getBuilder().newInstance(IMonomer.class);
    ...
    public void removeMonomer(String name)  {
        if (monomers.containsKey(name)) {
            IMonomer monomer = (IMonomer)monomers.get(name);
            this.remove(monomer);
    ...
    public Object clone() throws CloneNotSupportedException {
        NNStrand clone = (NNStrand)super.clone();
        clone.monomers.clear();
        for (Iterator<String> iter = clone.getMonomerNames().iterator(); iter.hasNext();) {
            IMonomer monomerClone = (IMonomer)(clone.getMonomer(iter.next().toString()).clone());

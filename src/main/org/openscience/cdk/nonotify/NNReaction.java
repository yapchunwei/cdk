 ...
 * @cdk.githash
 */
public class NNReaction extends NNChemObject implements IReaction {
     ...
     */
    public NNReaction() {
        this.reactants = getBuilder().newInstance(IMoleculeSet.class);
        this.products = getBuilder().newInstance(IMoleculeSet.class);
        this.agents = getBuilder().newInstance(IMoleculeSet.class);
        this.map = new IMapping[growArraySize];
        mappingCount = 0;
    ...
    public Object clone() throws CloneNotSupportedException {
        NNReaction clone = (NNReaction)super.clone();
        // clone the reactants, products and agents
        clone.reactants = (IMoleculeSet)reactants.clone();
        clone.agents = (IMoleculeSet)agents.clone();
        clone.products = (IMoleculeSet)products.clone();
        ...
        for (int i = 0; i < reactants.getMoleculeCount(); ++i) {
            IMolecule mol = (IMolecule)reactants.getMolecule(i);
            IMolecule mol2 = (IMolecule)clone.reactants.getMolecule(i);
            for (int j = 0; j < mol.getAtomCount(); ++j) atomatom.put(mol.getAtom(j), mol2.getAtom(j));
        }
        
        // clone the maps
        clone.map = new IMapping[map.length];
        for (int f = 0; f < mappingCount; f++) {
            clone.map[f] = getBuilder().newInstance(IMapping.class,
